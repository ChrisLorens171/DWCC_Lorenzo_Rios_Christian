const items = [
  ...
  $d.addEventListener("DOMContentLoaded", ev=>{
    ...
    function renderCategorias(categorias) {
      /* $categorias.innerHTML=categorias.map((categoria,i)=>`
        <button type="button" class="btn-categoria" data-categoria>
          ${categoria}
        </button>
      `).join('') */
  
      $categorias.innerHTML=categorias.reduce((anterior,actual,i)=>anterior+`
        <button type="button" class="btn-categoria" data-categoria">
          ${actual}
        </button>
    ...
    $categorias.addEventListener("click",ev=> {
      ...
      if(ev.target.tagName=="BUTTON") {
        ...
        if (categoria == "todos") {
          ...
          const productosFitlrados = items.filter(producto=>producto.category==categoria)
          renderProductos(productosFitlrados)
        }
      }
    ...
    function getCategorias(productos) {
      ...
      //return new Array(new Set(productos.map(producto=>producto.category)))
      
      /*
      let categorias = ["Todos"]
      for(let i=0;i<productos.length;i++) {
        if(!(categorias.includes(productos[i].category))) {
          categorias.push(productos[i].category)
        }
      }
      */
    } 
